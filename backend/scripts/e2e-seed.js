...
async function runE2ESeed(opts) {
    const client = opts?.client ?? exports.prisma;
    ...
    if (isFull) {
        ...
        await client.loyaltyCoupon.create({
            data: {
                ...
                discountType: 'FIXED',
                value: new client_1.Prisma.Decimal(10),
                active: true,
            },
        });
        const now = Date.now();
        ...
        // teardown (orders & finance events)
        {
            ...
            const exchangeOrderIdsToDelete = Array.from(new Set([
        ...
        const assertUuidLike = (name, value) => {
        ...
        if (!balance) {
            throw new Error(`E2E fixture invalid: inventoryBalance missing for replenishment fixture referenceId=${E2E_REPLENISH_SALE_REF} productId=${product.id} warehouseId=${warehouse.id}`);
        }
        // UI defaults expect onHand low enough so suggestedQty > 0.
        if (balance.onHandQty > 0) {
            throw new Error(`E2E fixture invalid: replenishment fixture expects inventoryBalance.onHandQty=0 (referenceId=${E2E_REPLENISH_SALE_REF}), got=${balance.onHandQty}`);
        }
        ...
        if (replSales.length < 1) {
            throw new Error(`E2E fixture invalid: replenishment suggestions missing inventoryEvent type=SALE_OUT referenceId=${E2E_REPLENISH_SALE_REF} count=${replSales.length}`);
        }
        const totalSold = replSales.reduce((s, r) => s + Math.abs(r.quantity), 0);
        if (totalSold <= 0) {
            throw new Error(`E2E fixture invalid: replenishment suggestions totalSold must be > 0 (referenceId=${E2E_REPLENISH_SALE_REF}), totalSold=${totalSold} samples=${replSales.length}`);
        }
        ...
        if (outOfLookback.length) {
            throw new Error(`E2E fixture invalid: replenishment suggestions occurredAt out of lookback window (referenceId=${E2E_REPLENISH_SALE_REF}) expected>=${minOccuredAt.toISOString()} sample=${outOfLookback.join(',')}`);
        }
        ...
        if (suggestedQty <= 0) {
            throw new Error(`E2E fixture invalid: replenishment suggestions suggestedQty must be > 0 (referenceId=${E2E_REPLENISH_SALE_REF}), suggestedQty=${suggestedQty} delta=${delta} onHandQty=${balance.onHandQty}`);
        }
        ...
        const inRange = expEvents.filter((e) => e.expiryDate && e.expiryDate >= expiringFrom && e.expiryDate <= expiringTo);
        ...
        if (expAllCount < 1 || expAllSumQty <= 0) {
            throw new Error(`E2E fixture invalid: expiring inventory missing/empty (batchCode=${E2E_EXPIRING_INVENTORY_BATCH} referenceId=${E2E_EXPIRING_INVENTORY_REF}) allCount=${expAllCount} allSumQty=${expAllSumQty}`);
        }
        if (inRangeCount < 1 || inRangeSumQty <= 0) {
            ...
            throw new Error(`E2E fixture invalid: expiring inventory batch must fall within expiring window (batchCode=${E2E_EXPIRING_INVENTORY_BATCH} referenceId=${E2E_EXPIRING_INVENTORY_REF}) expected expiryDate in [${expiringFrom.toISOString()}, ${expiringTo.toISOString()}], inRangeCount=${inRangeCount} inRangeSumQty=${inRangeSumQty} expirySamples=${expirySamples.join(',')}`);
        }
        ...
        if (rn.status !== 'COMPLETED') {
            throw new Error(`E2E fixture invalid: receivingNote must be COMPLETED for return-to-supplier (receiptNumber=${E2E_RN_RECEIPT_NUMBER}) gotStatus=${rn.status}`);
        }
        ...
        if (rnLines.length < 1) {
            throw new Error(`E2E fixture invalid: receivingNoteLine missing for return-to-supplier (receiptNumber=${E2E_RN_RECEIPT_NUMBER}) linesCount=${rnLines.length}`);
        }
        const eligibleLine = rnLines.find((l) => (l.qualifiedQty ?? 0) >= 1);
        if (!eligibleLine) {
            throw new Error(`E2E fixture invalid: receivingNoteLine qualifiedQty must allow return qty=1 (receiptNumber=${E2E_RN_RECEIPT_NUMBER}) qualifiedQtys=${rnLines
        ...
        if (remainingReturnableQty < 1) {
            throw new Error(`E2E fixture invalid: receivingNote returnable qty must be >=1 (receiptNumber=${E2E_RN_RECEIPT_NUMBER}) receivingNoteLineId=${eligibleLine.id} qualifiedQty=${eligibleLine.qualifiedQty} alreadyReturned=${alreadyReturnedQty} remainingReturnable=${remainingReturnableQty}`);
        }
        ...
        if (!pl) {
            throw new Error(`E2E fixture invalid: purchaseOrderLine missing for receivingNoteLineId=${eligibleLine.id} purchaseOrderLineId=${eligibleLine.purchaseOrderLineId}`);
        }
        ...
        if (!productById || !warehouseById) {
            throw new Error(`E2E fixture invalid: product/warehouse missing for receivingNote return-to-supplier (receiptNumber=${E2E_RN_RECEIPT_NUMBER}) productId=${pl.productId} warehouseId=${po.warehouseId}`);
        }
        const singleCount = await client.product.count({ where: { barcode: E2E_BARCODE_SINGLE } });
        if (singleCount !== 1) {
            throw new Error(`E2E fixture invalid: barcode single fixture key=${E2E_BARCODE_SINGLE} productCount=${singleCount} (expected 1)`);
        }
        const multiCount = await client.product.count({ where: { barcode: E2E_BARCODE_MULTI } });
        if (multiCount < 2) {
            throw new Error(`E2E fixture invalid: barcode multi fixture key=${E2E_BARCODE_MULTI} productCount=${multiCount} (expected >=2)`);
        }
        ...
        if (!srcOrder || !derivedOrder || derivedOrder.exchangeFromOrderId !== E2E_EX_SOURCE_ORDER_ID) {
            throw new Error(`E2E fixture invalid: exchange settlement linkage broken expected exchangeFromOrderId=${E2E_EX_SOURCE_ORDER_ID} derivedOrderId=${E2E_EX_DERIVED_ORDER_ID} derived.exchangeFromOrderId=${derivedOrder?.exchangeFromOrderId ?? 'null'}`);
        }
        const refundCount = await client.financeEvent.count({
            where: { referenceId: E2E_EX_SOURCE_ORDER_ID, type: 'SALE_REFUND' },
        });
        if (refundCount < 1) {
            throw new Error(`E2E fixture invalid: exchange source order missing financeEvent type=SALE_REFUND referenceId=${E2E_EX_SOURCE_ORDER_ID} count=${refundCount} (expected >=1)`);
        }
        ...
        const distinctRefIds = Array.from(new Set(financeRefs.map((r) => (r.referenceId ?? '').trim()).filter((x) => x)));
        if (distinctRefIds.length < 1) {
            throw new Error(`E2E fixture invalid: finance referenceId candidates missing referenceIdsExpected=[${[
                E2E_ORDER_ID,
                E2E_RN_ID,
                E2E_EX_SOURCE_ORDER_ID,
                E2E_EX_DERIVED_ORDER_ID,
        ...
        if (posSet.size < 1 && rnSet.size < 1) {
            throw new Error(`E2E fixture invalid: no finance referenceId resolvable to posOrder/receivingNote expectedDistinctRefs=[${distinctRefIds.join(',')}] posOrderCount=${posSet.size} receivingNoteCount=${rnSet.size}`);
        }
        ...
        if (!latestPos) {
            throw new Error(`E2E fixture invalid: missing latest posOrder finance event expectedReferenceIdIn=[${[E2E_ORDER_ID, E2E_EX_SOURCE_ORDER_ID].join(',')}]`);
        }
        if (latestRn && latestPos.occurredAt <= latestRn.occurredAt) {
            throw new Error(`E2E fixture invalid: posOrder finance event must be newer than receivingNote referenceId=${E2E_RN_ID} posOccurredAt=${latestPos.occurredAt.toISOString()} receivingNoteOccurredAt=${latestRn.occurredAt.toISOString()}`);
        }
        ...
        if (shouldRunCount < 1) {
            throw new Error(`E2E fixture invalid: dispatch enabled rule not runnable name=${E2E_DISPATCH_RULE_ENABLED_NAME} count=${shouldRunCount} (expected>=1)`);
        }
        ...
        if (disabledCount < 1) {
            throw new Error(`E2E fixture invalid: dispatch disabled rule missing name=${E2E_DISPATCH_RULE_DISABLED_NAME} count=${disabledCount} (expected>=1)`);
        }
        ...
        if (futureCount < 1) {
            throw new Error(`E2E fixture invalid: dispatch future rule runnable but should not name=${E2E_DISPATCH_RULE_FUTURE_NAME} count=${futureCount} (expected>=1 but nextRunAt>now)`);
        }
    }
    ...
    if (isFull) {
        ...
        console.log('E2E_SEED_SUMMARY', JSON.stringify({
            E2E_PROFILE: 'full',
