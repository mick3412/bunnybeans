...
2. **下方：本日變更紀錄（僅追加、不刪不改）**
   ...
   - 每次更新當日進度時，**只在此區塊追加**一筆，格式例如：
     - `- HH:MM 今日完成：新增 POS 整合測試`
     - `- HH:MM 更新：今日完成追加「業務錯誤碼」；卡點改為無`
   - **禁止刪除或改寫此區塊**，僅可追加新行。同一天多次更新即可保留時間序 log。
   - **時間必須真實**：行首 `HH:MM` 須為**實際寫入該筆紀錄的當下時間**（看系統時鐘，或終端執行 `date +%H:%M`）。**禁止隨意捏造時間**，以免 log 失序或誤導。若無法取得精確分鐘，可改用「日期 + 簡述」如 `- YYYY-MM-DD 晚間 更新：…`，但仍應反映真實提交時段，勿瞎掰。
