...
export const AdminCustomersPage: React.FC = () => {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-4 rounded-2xl border border-[#e2e8f0] bg-white p-6 shadow-sm" data-testid="e2e-admin-customers">
      <p className="text-sm text-muted">
        資料來源 <code className="rounded bg-[#f1f5f9] px-1 text-[#1e293b]">GET /customers?merchantId=</code>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-xl border border-[#e2e8f0] bg-white p-4 shadow-sm">
        ...
        <div>
          ...
          <select
            className="rounded-lg border border-[#e2e8f0] px-3 py-1.5 text-sm focus:border-[#0ea5e9] focus:ring-2 focus:ring-[#0ea5e9]/20"
            value={statusFilter}
        ...
        {exportPanelOpen && (
          <div className="flex w-full flex-wrap items-end gap-3 border-t border-[#e2e8f0] pt-3 mt-3">
            <span className="w-full text-xs font-semibold text-[#64748b]">分群名單匯出</span>
            <input
              type="text"
              className="w-56 rounded-lg border border-[#e2e8f0] bg-white px-3 py-1.5 text-sm font-mono focus:border-[#0ea5e9] focus:ring-2 focus:ring-[#0ea5e9]/20"
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={11} className="px-3 py-6 text-center text-[#64748b]">
                    {items.length === 0 ? '尚無會員資料' : '無符合篩選條件的會員'}
                  </td>
                </tr>
              ) : (
                filtered.map((r) => (
        ...
        <>
          ...
          <aside className="fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col border-l border-[#e2e8f0] bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-4 py-3">
              ...
              <button type="button" className="rounded px-2 py-1 text-[#64748b] hover:bg-[#f8fafc]" onClick={() => setContactsCustomer(null)}>關閉</button>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto p-4 space-y-4">
              {contactsLoading ? (
                ...
                <ul className="space-y-2">
                  {contactsItems.map((c) => (
                    <li key={c.id} className="rounded-lg border border-[#e2e8f0] bg-[#f8fafc] p-2 text-xs">
