...
test.describe('換貨 Phase2：差額/對帳旅程', () => {
  test('原單 → 換貨單 → 顯示差額與狀態（可 skip）', async ({ page }) => {
    ...
    if (!(await exchangeCardTitle.isVisible().catch(() => false))) {
    ...
    if (!(await derivedSection.isVisible().catch(() => false))) {
    ...
    if (!(await deltaBadge.isVisible().catch(() => false))) {
    ...
    await expect(page.getByText(/退款狀態：|補款狀態：/)).toBeVisible();
  });
});
